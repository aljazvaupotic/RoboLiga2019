...
ROBOT_ID = 0
SERVER_IP = "192.168.0.169" #spremeni na njihovega
...
#zagrabi jabolko
def take():
    motor_claw.run_to_rel_pos(position_sp = 210, speed_sp = 750)
...
def closest_home():
    ...
    return {'position': [top.x, (top.y+bottom.y)/2], 'type': 'home', 'id': 200}
def closest_enemy():
    if team_op_tag == 'team1':
        top = Point(basket_op['topRight'][0:2])
        bottom = Point(basket_op['bottomRight'][0:2])
    else:
        top = Point(basket_op['topLeft'][0:2])
        bottom = Point(basket_op['bottomLeft'][0:2])
    return {'position': [top.x, (top.y+bottom.y)/2], 'type': 'enemy', 'id': 200}
        ...
        if robot_alive and game_on:
            ...
            if state == State.IDLE:
                ...
                if len(good_apples) > 0:
                    if str(game_state[team_my_tag]['score']) == '0':
                        dist_middle = 1500
                        dist_bot = 2000
                        middle = Point([game_state['field']['topRight'][0]/2, game_state['field']['topRight'][1]/2])
                        for apple in good_apples:
                            dist = get_distance(middle, Point(apple['position'][0:2]))
                            dist2 = get_distance(robot_pos, Point(apple['position'][0:2]))
                            if dist < dist_middle and dist2 < dist_bot:
                                target = apple
                                dist_middle = dist
                                dist_bot = dist2
                        targetList = [target]
            ...
            #------------------------------------------------------------------
                ...
                if str(target['id']) == '100':
                    ...
                    if is_in_way(game_state['apples'], robot_pos, robot_dir, target):
                        target = {
                            'position': [robot_pos.x + (math.cos(math.radians(robot_dir + 30)) * 250), robot_pos.y + (math.sin(math.radians(robot_dir + 30)) * 150)],
                            'type': 'waypoint',
                            'id': 100
                        }
                        print("-----------------------SETTING WAYPOINT--------------------")
                        print("Gremo na: ["+str(target['position'][0])+", "+str(target['position'][1])+"]")
