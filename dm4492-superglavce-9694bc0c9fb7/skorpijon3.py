...
def return_bad(apples):
    bad = []
    for a in apples:
        if a['type'] == 'appleBad' and not in_op_basket(a):
            bad.append(a)
    return bad
            ...
            #------------------------------------------------------------------
                ...
                if str(target['id']) == '100': #waypoint
                    ...
                    if is_in_way(game_state['apples'], robot_pos, robot_dir, target):
                        target = {
                            'position': [robot_pos.x + (math.cos(math.radians(robot_dir + 30)) * 500), robot_pos.y + (math.sin(math.radians(robot_dir + 30)) * 400)],
                            'type': 'waypoint',
                            'id': 100
                        }
